...
sudo mysql -e "DROP DATABASE moodle_sand"
sudo mysqladmin create moodle_sand
echo "Refreshing database without logstore table..."
sudo mysqldump -v moodle_prod --no-data | sudo mysql moodle_sand
sudo mysqldump -v moodle_prod --no-create-info --ignore-table=moodle_prod.mdl_logstore_standard_log | sudo mysql moodle_sand
echo "Running upgrade script..."
sudo /rlscripts/moodle/moodle_upgrade --skip-sizecheck --skip-backup --skip-refresh /mnt/code/www/moodle_prod/
